...
- Пояснения, инструкции навыков (включая `description`), отображаемые названия и описания ролей, рабочие процессы, шаблоны и сценарии проверки пишите по-русски. Сохраняйте технические идентификаторы, ключи YAML, значения перечислений, пути, код и буквальные тестовые данные. Названия продуктов и устоявшиеся термины переводить не требуется. Язык результата определяется запросом пользователя.
